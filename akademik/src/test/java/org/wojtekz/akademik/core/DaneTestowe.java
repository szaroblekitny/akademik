...
public class DaneTestowe {
	...
	public <T> void wrzucTrocheDanychDoBazy(List<T> lista) {
		logg.debug("----->>> wrzucTrocheDanychDoBazy");
		...
		*/
		Type sup = lista.getClass().getGenericSuperclass();
		// Type typek = ((ParameterizedType)sup).getActualTypeArguments()[0];
		if (sup.getClass().equals(Pokoj.class)) {
		...
		if (sup.getClass().equals(Student.class)) {
